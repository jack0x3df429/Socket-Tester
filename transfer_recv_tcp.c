...
void* Data_Proc(void* pipefd) {
    ...
    char file_name_delay[7]="t_dly";
    char file_name_bitrate[7]="t_brt";
    char file_name_sec[7]="t_sec";
...
void receive(const int port) {
    ...
    struct timespec *recvd=(struct timespec *)malloc(sizeof(struct timespec));
    ...
    while ((bytesRead = recvmsg(clientfd, &msg, 0)) > 1) {
        ...
        while(pdata.bytesRead<MAX_BUFFER_SIZE){
          bytesRead=recv(clientfd,buffer+pdata.bytesRead,MAX_BUFFER_SIZE-pdata.bytesRead,0);
          if(bytesRead<0)
            continue;
          pdata.bytesRead+=bytesRead;
        }
        if(pdata.bytesRead<MAX_BUFFER_SIZE)
            continue;
        bytesRead_total+=pdata.bytesRead;
